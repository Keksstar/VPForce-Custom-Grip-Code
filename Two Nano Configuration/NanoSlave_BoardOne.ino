...
//To be configured for 16 button inputs two axes, combined with another board to be able to output 32 buttons with 4 axes to take advantage of full win wing profile. 

//THIS IS THE PROFILE FOR BOARD 1 WHICH WILL CONNECT DIRECTLY TO THE VPF MAIN BOARD. IT WILL NOT WORK ON ITS OWN AND WILL NEED BOARD 2 TO FUNCTION PROPERLY
...
volatile byte recbit1 = 10;
volatile byte recbit2 = 11; 
volatile byte reclowerbit1 = 12;
volatile byte recupperbit1 = 13; 
volatile byte reclowerbit2 = 14;
volatile byte recupperbit2 = 15;
...
ISR (PCINT0_vect)
{
  ...
  {
    pullcount = 1;
    SPDR = buttonbit1; //byte 1 buttons 0-8, from board 1
    pullcount++;
  }
}
...
ISR (SPI_STC_vect)
{
 ...
 if (pullcount == 2) // byte 2 buttons 9-16 from board 1
    {
      recbit1 = SPDR; //pull byte 1 buttons from other board, store here to send later 
      SPDR = buttonbit2; //send byte2 from this board to VPF
      pullcount++; 
    }
    else if (pullcount == 3) //byte 3 buttons 0-8 from board 2
    {
      recbit2 = SPDR; //pull axis 1 lowerbit from other board, store here to send later
      SPDR = recbit1; //send byte1 from other board to VPF
      pullcount++; 
    }
    else if (pullcount == 4) //byte 4 buttons 9-16 from board 2
    {
      reclowerbit1 = SPDR; //pull axis 1 upperbit from other board, store here to send later
      SPDR = recbit2; //send byte2 from other board to VPF
      pullcount++;
    }
    else if (pullcount == 5) //byte 5 buttons, none, set to 0xFF
    {
      recupperbit1 = SPDR; //pull axis 2 lowerbit from other board, store here to send later
      SPDR = 255;
      pullcount++; 
    }
    else if (pullcount == 6) //byte 6 buttons, none, set to 0xFF
    {
      reclowerbit2 = SPDR; //pull axis 2 upperbit from other board, store here to send later
      SPDR = 255;
      pullcount++;  
    }
    else if (pullcount == 7) //byte 7 reserved, none, set to 0x00
    {
      recupperbit2 = SPDR; 
      SPDR = 0;
      pullcount++;  
    }
    else if (pullcount == 8) //byte 8 reserved, set to 0x10 to eliminate axis to hat
    {
      SPDR = 16;
      pullcount++;  
    }
    else if (pullcount == 9) //byte 9 axis1b, signal from A6 on board 1
    {
    ...
    else if (pullcount == 10) //byte 10 axis1a, signal from A6 on board 1
    {
      //Swap the following two lines comments if axis is unused: 
      //SPDR = 0; 
      SPDR = upperbit1;
    ...
    else if (pullcount == 11) //byte 11 axis2b, signal from A7 on board 1
    {
      //Swap the following two lines comments if axis is unused: 
      //SPDR = 0; 
      SPDR = lowerbit2;
    ...
    else if (pullcount == 12) //byte 12 axis2a, signal from A7 on board 1
    {
      //Swap the following two lines comments if axis is unused: 
      //SPDR = 0; 
      SPDR = upperbit2;
    ...
    else if (pullcount == 13) //byte 13 axis3b, signal from A6 on board 2
    {
      //If this axis is not being used, swap in board two code, not here
      SPDR = reclowerbit1;
      pullcount++;  
    }
    else if (pullcount == 14) //byte 14 axis3a, signal from A6 on board 2
    {
      //If this axis is not being used, swap in board two code, not here
      SPDR = recupperbit1;
      pullcount++;  
    }
    else if (pullcount == 15) //byte 15 axis4b, signal from A7 on board 2
    {
      //If this axis is not being used, swap in board two code, not here
      SPDR = reclowerbit2;
      pullcount++;  
    }
    else if (pullcount == 16) //byte 16 axis4a, signal from A7 on board 2
    {
      //If this axis is not being used, swap in board two code, not here
      SPDR = recupperbit2;
      pullcount++;  
